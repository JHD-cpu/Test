package com.example.testSpring.config;

import com.alipay.api.AlipayApiException;
import com.alipay.api.AlipayClient;
import com.alipay.api.AlipayConfig;
import com.alipay.api.DefaultAlipayClient;
import org.springframework.context.annotation.Bean;
import org.springframework.context.annotation.Configuration;

@Configuration
public class AliPayConfig {


    // 支付宝appId
    public static final String app_id_dev= "2021000117658538";

    public static final String app_id= "2021003187606104";
    // 商户账号
    public static final String merchant_account = "2088621955813217";
    // 支付宝网关
    public static final String alipay_gateway_address_dev = "https://openapi.alipaydev.com/gateway.do";

    public static final String alipay_gateway_address = "https://openapi.alipay.com/gateway.do";

    // 支付宝公钥
    public static final String alipay_public_key_dev =  "";

    public static final String alipay_public_key= "";
    // 应用私钥:
    public static final String app_private_key = "";
    // 签名方式
    public static final String sign_type = "RSA2";

    public static final String aes_key_dev ="e5A+G8i785XtT6O/rUBnaQ==";
    public static final String aes_key = "wouQP85VfOQtaGwrOn9CgA==";
    // 字符编码格式
    public static final String charset = "utf-8";
    public static final String return_url = "https://3b973ce7.r15.cpolar.top";
    public static final String notify_url = "https://3b973ce7.r15.cpolar.top";

    public static final String rootCertPath=Thread.currentThread().getContextClassLoader().getResource("cret/alipayRootCert.crt").getPath();
    public static final String appCertPath=Thread.currentThread().getContextClassLoader().getResource("cret/appPublicCert.crt").getPath();
    public static final String alipayPublicCertPath=Thread.currentThread().getContextClassLoader().getResource("cret/alipayPublicCert.crt").getPath();

    @Bean
    public AlipayConfig alipayConfig(){
        AlipayConfig alipayConfig = new AlipayConfig();
        alipayConfig.setReadTimeout(10000);
        alipayConfig.setConnectTimeout(10000);
        alipayConfig.setAlipayPublicKey(alipay_public_key_dev);
        alipayConfig.setPrivateKey(app_private_key);
        alipayConfig.setCharset(charset);
        alipayConfig.setAppId(app_id_dev);
        alipayConfig.setSignType(sign_type);
        alipayConfig.setServerUrl(alipay_gateway_address_dev);
        alipayConfig.setEncryptKey(aes_key_dev);
        /*
        alipayConfig.setRootCertPath(rootCertPath);
        alipayConfig.setAppCertPath(appCertPath);
        alipayConfig.setAlipayPublicCertPath(alipayPublicCertPath);*/
        return alipayConfig;
    }
    @Bean
    public AlipayClient alipayClient(AlipayConfig alipayConfig)  {
        try {
            return new DefaultAlipayClient(alipayConfig);
        } catch (AlipayApiException e) {
            e.printStackTrace();
        }
        return null;
    }
}
